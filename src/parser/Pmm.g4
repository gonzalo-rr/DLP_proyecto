...
expression returns [Expression ast]: func_invocation { $ast = $func_invocation.ast; }
            ...
            ;
ids returns [List<Var> ast = new ArrayList<Var>()]: ID { if ($ast.stream().anyMatch(variable -> variable.name.equals($ID.text))) { new ErrorType("Duplicated identifier" + $ID.text, $ID.getLine(), $ID.getCharPositionInLine() + 1); } else { $ast.add(0, new Var($ID.text, $ID.getLine(), $ID.getCharPositionInLine() + 1)); } }
     | ID ',' ids { if ($ids.ast.stream().anyMatch(variable -> variable.name.equals($ID.text))) { new ErrorType("Duplicated identifier " + $ID.text, $ID.getLine(), $ID.getCharPositionInLine() + 1); } else { $ids.ast.add(0, new Var($ID.text, $ID.getLine(), $ID.getCharPositionInLine() + 1)); } $ast = $ids.ast; }
     ;
